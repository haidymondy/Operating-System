...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int o_id = get_semaphore_object_ID(ownerEnvID, semaphoreName);
	//if(ownerEnvID!=NULL)
	if(o_id == E_SEMAPHORE_EXISTS)
	{return E_SEMAPHORE_EXISTS;}
	else
	{   struct Semaphore * new_sem = NULL;
		int obj_id = allocate_semaphore_object(&new_sem);
		if(obj_id == E_NO_SEMAPHORE)
		{return E_NO_SEMAPHORE;}
		else
		{
//			struct Semaphore obj = semaphores[o_id];
			// inint obj
//			obj.name = &semaphoreName;
			strcpy(semaphores[obj_id].name, semaphoreName);
			semaphores[obj_id].ownerID = ownerEnvID;
			semaphores[obj_id].value = initialValue;
			//semaphores[&obj.name];
...
//	struct Semaphore * new_sem = NULL;
	int o_id = get_semaphore_object_ID( ownerEnvID, semaphoreName);
	semaphores[o_id].value--;
	if(semaphores[o_id].value < 0)
	{
//		enqueue(myenv, &semaphores[o_id].env_queue);
		enqueue(&semaphores[o_id].env_queue, curenv);
...
//			struct Env_Queue myQueue ;
			env = dequeue(&semaphores[o_id].env_queue);
//			env = dequeue(&myQueue);
