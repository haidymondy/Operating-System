...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int n=get_share_object_ID(ownerID,shareName);
	if (n!=E_SHARED_MEM_NOT_EXISTS)
		return -80;
	int shareObjectID ;
	struct Share* msh=NULL;
	shareObjectID = allocate_share_object(&msh);
	if(shareObjectID == E_NO_SHARE)
		return E_NO_SHARE;
	/////////////////////////////////////////////////////////////////////////////////
	int c=allocate_chunk(myenv->env_page_directory,(uint32)virtual_address,size,PERM_WRITEABLE|PERM_USER);
	if (c==-1)
		return -1;
	shares[shareObjectID].ownerID=ownerID;
	strcpy(shares[shareObjectID].name,shareName);
	shares[shareObjectID].isWritable=isWritable;
	shares[shareObjectID].size=size;
	shares[shareObjectID].references=1;
	shares[shareObjectID].empty=0;
	int count = 0;

	for(int i = (uint32)virtual_address ; i < (uint32)virtual_address+size ;i+=PAGE_SIZE ){
		uint32 * ptr_page=NULL;
		struct FrameInfo* info = get_frame_info(myenv->env_page_directory,i,&ptr_page);
		add_frame_to_storage(shares[shareObjectID].framesStorage,info,count);
		count++;
	}
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int index=0;
	for(int i = (uint32)virtual_address ; i < (uint32)virtual_address+size ;i+=PAGE_SIZE ){
		uint32 perms=shares[shareObjectID].isWritable;
		uint32 * ptr_page=NULL;
		perms=PERM_USER|(perms);
		struct FrameInfo* info = get_frame_from_storage(shares[shareObjectID].framesStorage,index);
		if(shares[shareObjectID].isWritable==0)
			map_frame(myenv->env_page_directory,info,i,perms);
		else
		{
	...
	shares[shareObjectID].references++;
	return shareObjectID;
}
